...
def clean_simple_inplace(mesh: trimesh.Trimesh):
    ...
    mesh.merge_vertices()
    mesh.update_faces(mesh.nondegenerate_faces()) #new version of trimesh has this function to remove degenerate faces, but it doesn't update the mesh in-place, so we need to call update_faces again
    mesh.update_faces(mesh.unique_faces()) # remove duplicate faces, but it doesn't update the mesh in-place, so we need to call update_faces again
