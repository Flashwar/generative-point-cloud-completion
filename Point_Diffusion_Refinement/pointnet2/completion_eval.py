...
def evaluate(net, testloader, diffusion_hyperparams, print_every_n_steps=200, parallel=True,
    ...
    if not dataset in ['shapenet', 'shapenet_pytorch', 'mvp_dataset', 'shapenet_chunk', 'mvp40', 'partnet', 'realpc_dataset']:
         raise Exception('%s dataset is not supported' % dataset)
    if use_a_precomputed_XT:
        assert task == 'completion'
        assert dataset in ['mvp_dataset', 'shapenet_chunk', 'mvp40', 'partnet', 'realpc_dataset'] # right now we only implemented this feature for mvp dataset
    if augment_data_during_generation:
        assert task == 'completion'
        assert dataset in ['mvp_dataset', 'shapenet_chunk', 'mvp40', 'partnet', 'realpc_dataset']
    if dataset == 'shapenet' or dataset=='shapenet_pytorch':
        total_meta = []
    elif dataset in ['mvp_dataset', 'shapenet_chunk', 'mvp40', 'partnet', 'realpc_dataset']:
    ...
    if save_generated_samples:
        ...
        if dataset in ['mvp_dataset', 'shapenet_chunk', 'mvp40', 'partnet', 'realpc_dataset']:
    ...
    for idx, data in enumerate(testloader):
        ...
        if task == 'refine_completion':
            ...
            if save_multiple_t_slices:
                assert dataset in ['mvp_dataset', 'shapenet_chunk', 'mvp40', 'partnet', 'realpc_dataset']# shapenet is not supported yet
        ...
        if save_generated_samples:
            if dataset in ['shapenet', 'shapenet_pytorch']:
                ...
                elif dataset == 'realpc_dataset':
                    save_file = os.path.join(save_dir, 'realpc_generated_data_%dpts.h5' % num_points)
                ...
                if save_multiple_t_slices:
                    ...
                    for t in t_slices:
                        if dataset == 'mvp_dataset':
                            ...
                            t_save_file = os.path.join(save_dir, 'realpc_generated_data_%dpts_T%d.h5' %  (num_points, t))
