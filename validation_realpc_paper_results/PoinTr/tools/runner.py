...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if dataset_name == 'PCN' or dataset_name == 'Completion3D' or dataset_name == 'Projected_ShapeNet' or dataset_name == 'Catenary':
                partial = data[0].cuda()
                gt = data[1].cuda()
...
def validate(base_model, test_dataloader, epoch, ChamferDisL1, ChamferDisL2, val_writer, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if interval == 0 or (idx+1) % interval == 0:
...
def test(base_model, test_dataloader, ChamferDisL1, ChamferDisL2, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if dataset_name == 'PCN' or dataset_name == 'Projected_ShapeNet' or dataset_name == 'Catenary':
                partial = data[0].cuda()
                gt = data[1].cuda()
