...
class Dataset(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if 'n_renderings' in self.options:
            file_path = sample.get('partial_cloud_path', [])
            if type(file_path) == list and len(file_path) > 0:
                rand_idx = random.randint(0, len(file_path) - 1) if self.options['shuffle'] else 0
            else:
                rand_idx = 0
        # load required data
        ...
        # Normalisierung: Zentrierung und Skalierung auf Einheitskugel
        if 'gtcloud' in data:
            centroid = data['gtcloud'].mean(axis=0)
            max_dist = np.max(np.sqrt(np.sum((data['gtcloud'] - centroid) ** 2, axis=1)))
            if max_dist > 0:
                for key in data:
                    data[key] = (data[key] - centroid) / max_dist
    ...
    def _get_file_list(self, cfg, subset, n_renderings=1):
        ...
        for dc in self.dataset_categories:
            ...
            for s in tqdm(samples, leave=False):
                ...
                if subset == 'test':
                    # for testing use the first partial scan
                    file_list.append({
                    'taxonomy_id': dc['taxonomy_id'],
                    'model_id': s,
                    'partial_cloud_path':
                        cfg.DATASETS.SHAPENET.PARTIAL_POINTS_PATH %
                        (subset, dc['taxonomy_id'], s, 0),
                    'gtcloud_path':
                    ...
                    # for training, read all available partial scans dynamically
                    ...
                    file_list.append({
                        'taxonomy_id': dc['taxonomy_id'],
                        'model_id': s,
                        'partial_cloud_path': partial_files,
                        'gtcloud_path':
                            cfg.DATASETS.SHAPENET.COMPLETE_POINTS_PATH %
                            (subset, dc['taxonomy_id'], s),
                    })
